...
def build_daily_equity_curve(trades):
    ...
    Notes
    -----
    This approximation lets us compute meaningful daily Sharpe / Sortino
    from a trade log alone. For higher fidelity, Mode 2 (signal + price
    series) supplies mark-to-market daily values.
    """
    if trades.empty:
...
def compute_metrics(trades):
    ...
    dstd = downside.std()
    sortino = (
        daily_returns.mean() / dstd * np.sqrt(TRADING_DAYS_PER_YEAR)
        if dstd and dstd > 0
        else 0.0
    )
    ...
    return {
        ...
        "calmar": float(calmar),
        "max_dd": max_dd,
        "max_intra_trade_dd": max_intra_trade_dd,
        "time_in_market": tim,
        "n_trades": n_trades,
        "win_rate": win_rate,
